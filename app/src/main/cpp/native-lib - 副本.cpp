...
/*
   if(!&SLAM)
   {
       ...
       ORB_SLAM2::System* tmpSLAM;
       tmpSLAM = new ORB_SLAM2::System("/sdcard/Download/SLAM/VOC/ORBvoc.bin","/sdcard/Download/SLAM/Calibration/PARAconfig.yaml",ORB_SLAM2::System::MONOCULAR,false);
       SLAM = tmpSLAM;
    ...
    LOGI("###gyj###SLAM initializing Start");
    // step 2 加载SLAM系统
    // Create SLAM system. It initializes all system threads and gets ready to process frames.
    ORB_SLAM2::System SLAM(
            "/storage/emulated/0/SLAM/VOC/ORBvoc.bin",                            // path_to_vocabulary
            "/storage/emulated/0/SLAM/Calibration/PARAconfig.yaml",                            // path_to_settings
            ORB_SLAM2::System::MONOCULAR,       // 单目模式
            false);
   LOGI("###gyj###SLAM initializing finished");
    ...
    LOGI("###gyj###TrackMonocular start");
    //ttrack 表示帧号
    cv::Mat pose = SLAM.TrackMonocular(*pMat,ttrack);
    LOGI("###gyj###TrackMonocular finished");
    end = clock();
    LOGI("###gyj###Get Pose Use Time=%f\n",((double)end-start)/CLOCKS_PER_SEC);
